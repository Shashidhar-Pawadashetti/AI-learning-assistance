  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const attempt = {
        ...
        questions,
        answers,
        score: Math.round((data.score / 100) * questions.length),
          ...
          {score !== null && (
            <div style={{marginTop:'2rem',background:'white',borderRadius:'16px',padding:'2rem',border:'1px solid #e2e8f0',boxShadow:'0 4px 12px rgba(0,0,0,0.08)'}}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, flexWrap: 'wrap', gap: 12 }}>
                <div style={{ flex: '1 1 200px' }}>
                  <p style={{fontSize:'1.5rem',fontWeight:'bold',margin:0,color:score >= 75 ? '#22c55e' : score >= 50 ? '#f59e0b' : '#ef4444'}}>
                    Score: {Math.round((score / 100) * questions.length)} / {questions.length}
                  </p>
                  <p style={{marginTop:'0.5rem',color:'#64748b'}}>XP Earned: +{Math.round((score / 100) * questions.length) * 10}</p>
                </div>
                <div style={{ flex: '0 0 auto' }}>
